...
def main_menu(user_id: int) -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton(text="👥 REFERRAL"), KeyboardButton(text="💳 METHODS")],
        [KeyboardButton(text="🎁 FREE PRODUCTS"), KeyboardButton(text="💰 DEPOSIT")],
        [KeyboardButton(text="📞 SUPPORT")],
        [KeyboardButton(text="💼 BALANCE"), KeyboardButton(text="🧾 MY ORDERS")],
    ]
    if is_admin(user_id):
...
def admin_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🎁 Free Products", callback_data="admin:free_products"),
        ...
        [
            InlineKeyboardButton(text="➕ Add Stock", callback_data="admin:add_stock"),
            InlineKeyboardButton(text="💳 Methods", callback_data="admin:methods"),
        ],
        [
...
def product_buy_keyboard(pid: str, stock_count: int) -> InlineKeyboardMarkup:
    rows = []
    if stock_count >= 1:
        rows.append([InlineKeyboardButton(text="Buy 1", callback_data=f"buy:{pid}:1")])
    if stock_count >= 2:
        rows.append([InlineKeyboardButton(text="Buy 2 — 5% OFF", callback_data=f"buy:{pid}:2")])
    if stock_count >= 5:
        rows.append([InlineKeyboardButton(text="Buy 5 — 10% OFF", callback_data=f"buy:{pid}:5")])
    if stock_count >= 10:
        rows.append([InlineKeyboardButton(text="Buy 10 — 15% OFF", callback_data=f"buy:{pid}:10")])
...
async def show_products(message: Message) -> None:
    ...
    cursor = products.find({"active": {"$ne": False}}).sort("name", ASCENDING)
...
async def user_view_product(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(
        ...
        reply_markup=product_buy_keyboard(pid, stock_count),
    )
...
async def free_canva_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"🎁 {bold('Free Canva Pro Request')}\n\n"
        "Send your Gmail address. Admin will add this Gmail to Canva Pro manually.\n\n"
...
async def save_canva_request(message: Message, state: FSMContext) -> None:
    ...
    result = await canva_requests.insert_one(doc)
    await state.clear()
    await message.answer(
        f"✅ Canva request sent.\n\nGmail: {code(gmail)}\nStatus: pending",
        reply_markup=main_menu(message.from_user.id),
    )
    for admin_id in ADMIN_IDS:
        await safe_send_message(
            admin_id,
            f"📩 {bold('New Free Canva Request')}\n\n"
            f"Request ID: {code(result.inserted_id)}\n"
            f"User: {code(message.from_user.id)} @{escape(message.from_user.username or '')}\n"
            f"Gmail: {code(gmail)}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="✅ Mark Added", callback_data=f"canvareq:done:{result.inserted_id}"),
                InlineKeyboardButton(text="❌ Reject", callback_data=f"canvareq:reject:{result.inserted_id}"),
            ]]),
        )
...
async def show_content_methods(message: Message) -> None:
    ...
    await ensure_user(message)
    rows: List[List[InlineKeyboardButton]] = []
    async for method in content_methods.find({"active": {"$ne": False}}).sort("created_at", DESCENDING):
...
async def buy_content_method(callback: CallbackQuery) -> None:
    ...
    result = await orders.insert_one({"user_id": callback.from_user.id, "username": callback.from_user.username, "product_id": mid, "product_name": f"METHOD: {method.get('name')}", "quantity": 1, "subtotal": price, "discount": 0, "total": price, "delivered": [method.get("delivery", "")], "status": "completed", "created_at": utcnow()})
    delivery_type = method.get("delivery_type", "text")
    delivery = method.get("delivery", "")
    ...
    if delivery_type == "video":
        await bot.send_video(callback.from_user.id, delivery, caption="🎬 Your method video")
    elif delivery_type == "document":
        await bot.send_document(callback.from_user.id, delivery, caption="📁 Your method file")
    else:
        await callback.message.answer(f"📄 Your method:\n\n{escape(delivery)}")
    await callback.answer("Delivered!")
...
async def deposit_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("💰 Enter deposit amount in USDT:")
...
async def deposit_amount(message: Message, state: FSMContext) -> None:
    ...
    rows = []
    async for method in payment_methods.find({}).sort("default", DESCENDING):
...
@router.message(F.text == "🎁 FREE PRODUCTS")
async def show_free_products(message: Message) -> None:
    ...
    if not rows:
        await message.answer("No free products available yet.")
        return
    await message.answer(f"🎁 {bold('Free Products')}\n\nUse referral points or USDT balance.", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def buy_free_product(callback: CallbackQuery) -> None:
    ...
    result = await orders.insert_one({"user_id": callback.from_user.id, "username": callback.from_user.username, "product_id": pid, "product_name": f"FREE: {product.get('name')}", "quantity": 1, "subtotal": total, "discount": 0, "total": total, "paid_with": paid_with, "delivered": [item], "status": "completed", "created_at": utcnow()})
...
async def admin_free_products(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback):
        return
    await callback.message.answer("🎁 Free Products Admin", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Add Free Product", callback_data="fpadmin:add")],
        [InlineKeyboardButton(text="📦 List Free Products", callback_data="fpadmin:list")],
...
async def add_free_product_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer("Send free product name:")
    await callback.answer()
...
async def add_free_product_name(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Send USDT price for this free product:")
...
async def add_free_product_desc(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(f"✅ Free product added. ID: {code(result.inserted_id)}")
...
async def list_free_products_admin(callback: CallbackQuery) -> None:
    ...
    async for p in free_products.find({}).sort("created_at", DESCENDING):
        found = True
        await callback.message.answer(f"🎁 {bold(p.get('name'))}\nID: {code(p['_id'])}\nUSDT: {bold(money(p.get('price', 0)))}\nPoints: {bold(p.get('points', 0))}\nStock: {bold(len(p.get('stock', [])))}")
    if not found:
        await callback.message.answer("No free products.")
    await callback.answer()
...
async def add_free_stock_pid(message: Message, state: FSMContext) -> None:
    ...
    product = await free_products.find_one({"_id": oid}) if oid else None
    if not product:
        await message.answer("Free product not found.")
        return
    await state.update_data(product_id=pid)
    await state.set_state(AddFreeStockState.stock)
    await message.answer("Send stock lines, one item per line:")
...
async def add_free_stock_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(f"✅ Added {len(lines)} free product stock items.")
...
async def add_content_method_desc(message: Message, state: FSMContext) -> None:
    if not await admin_only_message(message):
        return
    await state.update_data(description=truncate(message.text or '', MAX_PRODUCT_DESCRIPTION))
    await state.set_state(AddContentMethodState.delivery)
    await message.answer("Send delivery content now: text, document/file, or video.")
...
async def add_content_method_video(message: Message, state: FSMContext) -> None:
    await save_content_method_delivery(message, state, "video", message.video.file_id)
...
async def add_content_method_document(message: Message, state: FSMContext) -> None:
    await save_content_method_delivery(message, state, "document", message.document.file_id)

@router.message(AddContentMethodState.delivery)
async def add_content_method_text(message: Message, state: FSMContext) -> None:
    await save_content_method_delivery(message, state, "text", message.text or "")

async def save_content_method_delivery(message: Message, state: FSMContext, delivery_type: str, delivery: str) -> None:
    if not await admin_only_message(message):
        return
    data = await state.get_data()
    result = await content_methods.insert_one({"name": data["name"], "price": float(data["price"]), "description": data["description"], "delivery_type": delivery_type, "delivery": delivery, "active": True, "sales": 0, "created_at": utcnow(), "updated_at": utcnow()})
    await state.clear()
    await message.answer(f"✅ Paid method added. ID: {code(result.inserted_id)}")
...
async def list_content_methods_admin(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback):
        return
    found = False
    async for m in content_methods.find({}).sort("created_at", DESCENDING):
        found = True
        await callback.message.answer(f"🎬 {bold(m.get('name'))}\nID: {code(m['_id'])}\nPrice: {bold(money(m.get('price', 0)))}\nType: {bold(m.get('delivery_type', 'text'))}\nActive: {bold(m.get('active', True))}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🟢/🔴 Toggle", callback_data=f"cmadmin:toggle:{m['_id']}")]]))
    if not found:
