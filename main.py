...
def main_menu(user_id: int) -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton(text="🎁 FREE PRODUCTS"), KeyboardButton(text="💳 METHODS")],
        [KeyboardButton(text="👥 REFERRAL"), KeyboardButton(text="💰 DEPOSIT")],
...
async def menu_text_always_works(message: Message, state: FSMContext) -> None:
    ...
    elif message.text == "🎁 FREE PRODUCTS":
...
async def free_canva_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "Send your Gmail address first. After Gmail, bot will ask you to choose payment: USDT balance or referral points.\n\n"
...
async def buy_content_method(callback: CallbackQuery) -> None:
    ...
    result = await orders.insert_one({"user_id": callback.from_user.id, "username": callback.from_user.username, "product_id": mid, "product_name": f"METHOD: {method.get('name')}", "quantity": 1, "subtotal": price, "discount": 0, "total": price, "delivered": delivery_items, "status": "completed", "created_at": utcnow()})
...
async def admin_free_products(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer("🎁 Free Products Admin", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📦 List / Delete Free Products", callback_data="fpadmin:list")],
...
async def delete_free_product_admin(callback: CallbackQuery) -> None:
    ...
    product = await free_products.find_one({"_id": oid}) if oid else None
    if not product:
        await callback.answer("Free product not found.", show_alert=True)
        return
    await free_products.delete_one({"_id": product["_id"]})
    await callback.message.answer(f"🗑 Deleted free product: {bold(product.get('name'))}")
...
async def add_free_stock_pid(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Send multiple free product stock items at once. Put one item per line:")
...
async def finish_content_method_delivery(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not items:
        ...
        return
    result = await content_methods.insert_one({"name": data["name"], "price": float(data["price"]), "description": data["description"], "delivery_type": "multi", "delivery": "", "delivery_items": items, "active": True, "sales": 0, "revenue": 0.0, "created_at": utcnow(), "updated_at": utcnow()})
    await state.clear()
...
# V4 UPDATE NOTES
# ==========================================================
# - Free Products button swapped with Referral button in main menu.
# - Admin can delete free products from the free products list.
