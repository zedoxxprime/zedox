...
async def stockout_preorder_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        f"Product: {bold(data.get('product_name'))}\n"
        f"Accounts: {bold(data.get('quantity', 1))}\n"
        f"Subtotal: {bold(money(data.get('subtotal', 0)))}\n"
        f"Advance fee ({int(PREORDER_ADVANCE_FEE_PERCENT)}%): {bold(money(data.get('advance_fee', 0)))}\n"
        f"Total to pay now: {bold(money(data.get('estimated_total', 0)))}\n\n"
