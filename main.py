...
async def verify_binance_deposit_tx(txid: str, amount: float) -> Tuple[bool, str]:
    """Verify a completed Binance spot deposit by txId and amount using Binance SAPI.
    ...
    def _request() -> Tuple[bool, str]:
        ...
        for row in payload:
            ...
            if txid.lower() in row_tx.lower() and row_coin == BINANCE_COIN and row_amount + 1e-9 >= float(amount):
                return True, f"Verified {row_amount} {row_coin} deposit."
...
async def show_products(message: Message) -> None:
    ...
    for product in product_list:
        ...
        base = f"{product.get('name')} — {money(product.get('price', 0))}"
        if stock_count <= 0:
            label = f"{base} — Stockout"
        else:
            label = f"{base} — Stock {stock_count}"
...
@router.callback_query(F.data.startswith("stockoutpreorder:"))
...
async def stockout_preorder_quantity(message: Message, state: FSMContext) -> None:
    ...
    if not product:
        ...
        return
    details = f"Stockout preorder: {product.get('name')} | Quantity: {qty} account(s)"
    doc = {
        "user_id": message.from_user.id,
        "username": message.from_user.username,
        "details": details,
    ...
    await state.clear()
    await message.answer(
        f"✅ Stockout preorder sent to admin.\n\nPreorder ID: {code(short_id(preorder_id))}\nProduct: {bold(product.get('name'))}\nAccounts: {bold(qty)}\n\nAdmin will notify you with price/payment details.",
        reply_markup=main_menu(message.from_user.id),
    )
    await notify_admins(
        f"📝 {bold('New Stockout Preorder')}\n\nPreorder ID: {code(preorder_id)}\nUser: {code(message.from_user.id)} @{escape(message.from_user.username or 'None')}\nProduct: {bold(product.get('name'))}\nAccounts: {bold(qty)}\nPayment: {bold('Not requested yet')}",
        reply_markup=preorder_admin_keyboard(preorder_id),
    )
...
async def deposit_binance_auto_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        f"Amount: {bold(money(amount))}\n"
        f"Coin: {bold(BINANCE_COIN)}\n\n"
        f"Send Binance transaction ID / TxID after payment. Bot will verify completed deposits automatically.",
        reply_markup=cancel_inline_keyboard(),
    )
    await callback.answer()
...
async def deposit_binance_auto_verify(message: Message, state: FSMContext) -> None:
    ...
    if len(txid) < 6:
        ...
        return
    ok, info = await verify_binance_deposit_tx(txid, amount)
    if not ok:
        await message.answer(f"❌ Auto verify failed.\n\nReason: {escape(info)}\n\nYou can try again or use manual screenshot deposit.", reply_markup=cancel_inline_keyboard())
        return
    doc = {
        "user_id": message.from_user.id,
        "username": message.from_user.username,
        "amount": amount,
        "method": "Binance Auto Verify",
        "txid": txid,
        "status": "approved",
        "auto_verified": True,
        "verify_info": info,
