...
async def preorder_save(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        f"✅ Preorder sent to admin.\n\nPreorder ID: {code(short_id(pid))}\nStatus: {bold('pending')}\n\nYou will get notification when admin answers.",
    ...
    await audit("preorder_created", message.from_user.id, {"preorder_id": pid})
