...
# ==========================================================
# X PREMIUM PLUS ACCOUNT SYSTEM - FULL WORKING PATCH
# ==========================================================
...
async def x_plan_keyboard(plans: Dict[str, Dict[str, Any]], user_id: int) -> InlineKeyboardMarkup:
    ...
    for key in X_PLAN_KEYS:
        ...
        rows.append([InlineKeyboardButton(
            text=f"🐦 {plan.get('name')} — {money(plan.get('price_usdt', 0))} / {plan.get('points', 0)} pts",
...
@router.message(StateFilter("*"), F.text.in_({"🐦 X PREMIUM PLUS", "X PREMIUM PLUS", "X Premium Plus", "x premium plus"}))
...
async def x_method_view(callback: CallbackQuery) -> None:
    ...
    if not await ensure_button_allowed(callback, "x_method"):
        return
    title = str(await get_setting("x_method_title", "X Premium Plus Method"))
