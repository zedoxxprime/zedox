...
def main_menu(user_id: int) -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(text="🛍️ PRODUCTS"), KeyboardButton(text="♻️ RESELL")],
        [KeyboardButton(text="🎁 FREE ITEMS"), KeyboardButton(text="📝 PREORDER")],
        [KeyboardButton(text="🐦 X PREMIUM PLUS")],
        [KeyboardButton(text="👥 REFERRAL")],
        [KeyboardButton(text="💳 METHODS"), KeyboardButton(text="💰 DEPOSIT")],
        [KeyboardButton(text="📞 SUPPORT")],
        [KeyboardButton(text="💼 BALANCE"), KeyboardButton(text="💱 EXCHANGE")],
    ]
    if is_admin(user_id):
...
def x_plan_keyboard(plans: Dict[str, Dict[str, Any]]) -> InlineKeyboardMarkup:
    rows = []
    for key in ["3m", "6m", "1y"]:
        plan = plans[key]
...
async def x_premium_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        reply_markup=x_plan_keyboard(plans),
    )
...
async def x_plan_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💎 Buy with Points", callback_data=f"xbuy:points:{key}")],
            [InlineKeyboardButton(text="💰 Buy with USDT Balance", callback_data=f"xbuy:balance:{key}")],
            [InlineKeyboardButton(text="📤 Pay Binance Manually", callback_data=f"xbuy:manual:{key}")],
