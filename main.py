...
async def show_products(message: Message) -> None:
    ...
    cursor = products.find({"active": {"$ne": False}}).sort("created_at", DESCENDING)
    found = False
...
async def free_canva_menu(message: Message) -> None:
    if not await security_check_event(message):
        return
    user = await ensure_user(message)
    ...
    canva = await products.find_one({"name": {"$regex": r"^canva pro$", "$options": "i"}})
    stock_count = len(canva.get("stock", [])) if canva else 0

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"Redeem with {points_required} points", callback_data="canva:points")],
        [InlineKeyboardButton(text=f"Buy with {money(price)}", callback_data="canva:usdt")],
    ])
    ...
    await message.answer(
        ...
        f"Admin tip: create a product named exactly {code('Canva Pro')} and add stock lines.",
        reply_markup=keyboard,
    )

@router.callback_query(F.data.startswith("canva:"))
async def buy_canva(callback: CallbackQuery) -> None:
    if not await security_check_event(callback):
        return
    ...
    product = await products.find_one({"name": {"$regex": r"^canva pro$", "$options": "i"}})
    if not product or not product.get("stock"):
        await callback.message.answer("❌ Canva Pro stock is unavailable. Contact support.")
        await callback.answer()
        return
    ...
    if mode == "points":
        required = int(await get_setting("canva_points_required", 10))
        if int(user.get("points", 0)) < required:
            await callback.answer("Not enough points.", show_alert=True)
            return
        await users.update_one({"user_id": callback.from_user.id}, {"$inc": {"points": -required, "total_orders": 1}})
        paid_text = f"{required} points"
    else:
        price = float(await get_setting("canva_price_usdt", 1.0))
        ...
        await users.update_one(
            {"user_id": callback.from_user.id},
            {"$inc": {"balance": -price, "total_spent": price, "total_orders": 1}},
        )
    ...
    await products.update_one(
        {"_id": product["_id"]},
        {"$pop": {"stock": -1}, "$inc": {"sales": 1, "revenue": total_value}},
    )
    ...
    result = await orders.insert_one(order_doc)
    await audit("canva_delivered", callback.from_user.id, {"order_id": str(result.inserted_id), "paid_with": paid_text})

    await callback.message.answer(
        f"✅ Canva Pro delivered!\n\n"
        f"Paid with: {bold(paid_text)}\n\n"
        f"Your data:\n{code(item)}",
        reply_markup=await contact_keyboard(),
    )
    await callback.answer("Delivered!")
...
async def show_payment_methods(message: Message) -> None:
    if not await security_check_event(message):
        return
    await ensure_user(message)

    cursor = payment_methods.find({}).sort("default", DESCENDING)
    found = False
    text = f"💳 {bold('Payment Methods')}\n\n"
    async for method in cursor:
        found = True
        star = "⭐ " if method.get("default") else ""
        text += f"{star}{bold(method.get('name'))}\n{code(method.get('details'))}\n\n"

    await message.answer(text if found else "No payment methods added yet.")
...
async def deposit_amount(message: Message, state: FSMContext) -> None:
    ...
    text = "💳 Choose payment method by sending its exact name:\n\n"
...
async def deposit_method(message: Message, state: FSMContext) -> None:
    ...
    method = await payment_methods.find_one({"name": {"$regex": f"^{re.escape(name)}$", "$options": "i"}})
    if not method:
        await message.answer("❌ Payment method not found. Send exact method name.")
        return
