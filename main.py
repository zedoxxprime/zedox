...
BUTTON_TITLES: Dict[str, str] = {
    ...
    "balance": "💼 BALANCE",
    "exchange": "💱 EXCHANGE",
    "x_plan_3m": "🐦 X Plan 3 Months",
    "x_plan_6m": "🐦 X Plan 6 Months",
    "x_plan_1y": "🐦 X Plan 1 Year",
    "x_method": "📝 X Method Button",
    "x_my_orders": "🧾 My X Orders Button",
...
def admin_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🐦 X Orders", callback_data="xadmin:orders"),
            InlineKeyboardButton(text="💰 X Payments", callback_data="xadmin:payments"),
        ],
        [
...
async def ensure_indexes() -> None:
    ...
    defaults = {
        ...
        "x_binance_id": "",
        "x_plans": {
            "3m": {"name": "X Premium Plus 3 Months", "months": 3, "price_usdt": 5.0, "points": 50},
            "6m": {"name": "X Premium Plus 6 Months", "months": 6, "price_usdt": 9.0, "points": 90},
            "1y": {"name": "X Premium Plus 1 Year", "months": 12, "price_usdt": 15.0, "points": 150},
        },
        "x_method_title": "X Premium Plus Method",
...
USER_MENU_TEXTS = {
    ...
    "🎁 FREE ITEMS", "📝 PREORDER", "🐦 X PREMIUM PLUS", "👑 ADMIN PANEL", "💱 EXCHANGE", "♻️ RESELL",
}
...
async def get_force_join_channels() -> List[str]:
    ...
    return [clean_username(str(x)) for x in (raw or []) if clean_username(str(x))]
...
async def force_join_required(user_id: int, scope: str = "all") -> bool:
    ...
    if not channels or mode in {"none", "off", "empty", "disabled"}:
        return False
...
async def send_force_join_message(message_or_callback: Message | CallbackQuery, scope: str = "all") -> bool:
    ...
    if not await force_join_required(user_id, scope):
        return False
    channels = await get_force_join_channels()
    text = "🔒 Please join required channel(s) first, then click ✅ I Joined."
    if isinstance(message_or_callback, CallbackQuery):
        await message_or_callback.message.answer(text, reply_markup=force_join_keyboard(channels))
        await message_or_callback.answer("Join required channel(s).", show_alert=True)
    else:
        await message_or_callback.answer(text, reply_markup=force_join_keyboard(channels))
    return True
...
async def menu_text_always_works(message: Message, state: FSMContext) -> None:
    ...
    elif message.text == "🐦 X PREMIUM PLUS":
...
async def setting_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    prompts = {
        ...
        "force_join_channels": "🔒 Send required channels separated by comma:\nExample: @channel1,@channel2\nSend off/none to disable channels.",
...
# ==========================================================
# X PREMIUM PLUS ACCOUNT SYSTEM
# ==========================================================

async def get_x_plans() -> Dict[str, Dict[str, Any]]:
    plans = await get_setting("x_plans", {})
    ...
    for k, v in defaults.items():
        plans.setdefault(k, v)
    return plans

async def x_plan_keyboard(plans: Dict[str, Dict[str, Any]], user_id: int) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    plan_visibility = {"3m": "x_plan_3m", "6m": "x_plan_6m", "1y": "x_plan_1y"}
    for key in ["3m", "6m", "1y"]:
        if not button_visible(plan_visibility[key], user_id):
            continue
        plan = plans[key]
        rows.append([InlineKeyboardButton(text=f"🐦 {plan['name']} — {money(plan.get('price_usdt', 0))} / {plan.get('points', 0)} pts", callback_data=f"x:plan:{key}")])
    if button_visible("x_method", user_id):
        rows.append([InlineKeyboardButton(text="📝 Buy X Premium Plus Method", callback_data="x:method")])
    if button_visible("x_my_orders", user_id):
        rows.append([InlineKeyboardButton(text="🧾 My X Orders", callback_data="x:myorders")])
    rows.append([InlineKeyboardButton(text="🏠 Main Menu", callback_data="global:main_menu")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

async def render_x_premium_menu(target: Message, state: FSMContext, user_id: int) -> None:
    """Render the X Premium Plus menu for the real user.

    This helper fixes callback/back-button flows where callback.message.from_user
    is the bot itself, not the user who clicked the inline button.
    """
    await state.clear()
    user = await get_user(user_id) or {}
    plans = await get_x_plans()
    await target.answer(
        f"🐦 {bold('X Premium Plus Store')}\n\n"
        f"Balance: {bold(money(user.get('balance', 0)))}\n"
        f"Points: {bold(user.get('points', 0))}\n\n"
        f"Choose a plan. You can buy using points, USDT balance, or manual Binance payment.",
...
@router.callback_query(F.data == "x:menu")
...
async def x_plan_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    plan_visibility = {"3m": "x_plan_3m", "6m": "x_plan_6m", "1y": "x_plan_1y"}
    if not await ensure_button_allowed(callback, plan_visibility.get(key, "x_premium")):
        return
    plans = await get_x_plans()
    plan = plans.get(key)
    if not plan:
    ...
    await callback.message.answer(
        ...
        f"After payment, bot will ask for the X username. Order stays pending until admin marks it done.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=(
            ([[InlineKeyboardButton(text="💎 Buy with Points", callback_data=f"xbuy:points:{key}")]] if button_visible("x_pay_points", callback.from_user.id) else []) +
            ([[InlineKeyboardButton(text="💰 Buy with USDT Balance", callback_data=f"xbuy:balance:{key}")]] if button_visible("x_pay_balance", callback.from_user.id) else []) +
            ([[InlineKeyboardButton(text="📤 Pay Binance Manually", callback_data=f"xbuy:manual:{key}")]] if button_visible("x_pay_manual", callback.from_user.id) else []) +
...
async def x_buy_internal(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    pay_visibility = {"points": "x_pay_points", "balance": "x_pay_balance"}
    if not await ensure_button_allowed(callback, pay_visibility.get(pay_type, "x_premium")):
        return
    plans = await get_x_plans()
    plan = plans.get(key)
    ...
    if not plan:
        await callback.answer("Plan not found.", show_alert=True)
        return
    if pay_type == "points" and int(user.get("points", 0)) < int(plan.get("points", 0)):
    ...
    await callback.message.answer("Send the X username/account handle where Premium Plus should be activated. Example: @username", reply_markup=cancel_inline_keyboard())
    await callback.answer()

@router.message(XUsernameState.plan_key)
async def x_username_received(message: Message, state: FSMContext) -> None:
    username = (message.text or "").strip().replace("https://x.com/", "").replace("https://twitter.com/", "")
    username = username.replace("@", "").strip()
    if not re.fullmatch(r"[A-Za-z0-9_]{1,30}", username):
        await message.answer("❌ Send a valid X username only, without spaces.", reply_markup=cancel_inline_keyboard())
        return
    data = await state.get_data()
    ...
    user = await get_user(message.from_user.id) or {}
    if not plan:
        await state.clear(); await message.answer("Plan not found.", reply_markup=main_menu(message.from_user.id)); return
    ...
    if pay_type == "points":
        ...
        if res.modified_count <= 0:
            await message.answer("❌ Not enough points now.", reply_markup=main_menu(message.from_user.id)); await state.clear(); return
        paid_value = points
    else:
        res = await users.update_one({"user_id": message.from_user.id, "balance": {"$gte": amount}}, {"$inc": {"balance": -amount}})
        if res.modified_count <= 0:
            await message.answer("❌ Not enough balance now.", reply_markup=main_menu(message.from_user.id)); await state.clear(); return
        paid_value = amount
    doc = {
        "user_id": message.from_user.id, "username": message.from_user.username, "x_username": username,
        "plan_key": key, "plan_name": plan.get("name"), "pay_type": pay_type,
        "amount_usdt": amount if pay_type == "balance" else 0, "points": points if pay_type == "points" else 0,
        "status": "pending", "created_at": utcnow(), "updated_at": utcnow(),
    }
    result = await x_orders.insert_one(doc)
    await state.clear()
    await message.answer(f"✅ Order created and payment deducted.\n\nOrder ID: {code(short_id(result.inserted_id))}\nPlan: {bold(plan.get('name'))}\nX Username: @{escape(username)}\nStatus: {bold('Pending admin completion')}", reply_markup=main_menu(message.from_user.id))
    await notify_admins(f"🐦 {bold('New X Premium Plus Order')}\n\nOrder ID: {code(str(result.inserted_id))}\nUser: {code(message.from_user.id)} @{escape(message.from_user.username or 'None')}\nPlan: {bold(plan.get('name'))}\nX Username: @{escape(username)}\nPaid with: {bold(pay_type)}\nValue: {bold(str(paid_value))}", reply_markup=x_order_admin_keyboard(str(result.inserted_id)))
...
async def x_manual_payment_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not await ensure_button_allowed(callback, "x_pay_manual"):
        return
    key = callback.data.split(":")[-1]
    plans = await get_x_plans(); plan = plans.get(key)
    if not plan:
        await callback.answer("Plan not found.", show_alert=True); return
    binance_id = str(await get_setting("x_binance_id", "") or await get_setting("binance_wallet_address", "") or "").strip()
    if not binance_id:
        await callback.message.answer("❌ Binance ID/wallet is not set by admin yet.", reply_markup=cancel_inline_keyboard()); await callback.answer(); return
...
async def x_payment_username(message: Message, state: FSMContext) -> None:
    ...
    if not re.fullmatch(r"[A-Za-z0-9_]{1,30}", username):
        await message.answer("❌ Send valid X username only.", reply_markup=cancel_inline_keyboard()); return
...
async def x_payment_txid(message: Message, state: FSMContext) -> None:
    txid = (message.text or "").strip()
    if len(txid) < 5:
        await message.answer("❌ TXID is too short. Send correct transaction ID.", reply_markup=cancel_inline_keyboard()); return
    existing = await x_payments.find_one({"txid": {"$regex": f"^{re.escape(txid)}$", "$options": "i"}})
    if existing:
        await message.answer("❌ This TXID was already submitted.", reply_markup=cancel_inline_keyboard()); return
...
async def create_x_payment(message: Message, state: FSMContext, file_id: str, file_type: str) -> None:
    data = await state.get_data(); plans = await get_x_plans(); plan = plans.get(data.get("plan_key"))
    if not plan:
        await state.clear(); await message.answer("Plan not found.", reply_markup=main_menu(message.from_user.id)); return
    doc = {
        "user_id": message.from_user.id, "username": message.from_user.username, "x_username": data.get("x_username"),
        "plan_key": data.get("plan_key"), "plan_name": plan.get("name"), "amount_usdt": float(plan.get("price_usdt", 0) or 0),
        "txid": data.get("txid"), "screenshot_file_id": file_id, "screenshot_type": file_type,
        "status": "pending", "created_at": utcnow(), "updated_at": utcnow(),
    }
    result = await x_payments.insert_one(doc)
    await state.clear()
    await message.answer(f"✅ Payment submitted.\n\nPayment ID: {code(short_id(result.inserted_id))}\nStatus: {bold('Pending admin approval')}", reply_markup=main_menu(message.from_user.id))
...
async def x_payment_approve(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback): return
    pid = callback.data.split(":", 2)[2]; oid = object_id(pid)
    payment = await x_payments.find_one({"_id": oid}) if oid else None
    if not payment or payment.get("status") != "pending":
        await callback.answer("Payment not pending.", show_alert=True); return
    ...
    await safe_send_message(int(payment["user_id"]), f"✅ Your X payment was approved.\n\nOrder ID: {code(short_id(res.inserted_id))}\nPlan: {bold(payment.get('plan_name'))}\nX Username: @{escape(payment.get('x_username'))}\nStatus: {bold('Pending admin completion')}")
    await callback.message.answer("✅ Payment approved and X order created.", reply_markup=x_order_admin_keyboard(str(res.inserted_id)))
    await callback.answer("Approved")
...
async def x_payment_reject_start(callback: CallbackQuery, state: FSMContext) -> None:
    if not await admin_only_callback(callback): return
    pid = callback.data.split(":", 2)[2]
    await state.set_state(XPaymentRejectState.reason); await state.update_data(payment_id=pid)
...
async def x_payment_reject_reason(message: Message, state: FSMContext) -> None:
    if not await admin_only_message(message): return
    data = await state.get_data(); oid = object_id(data.get("payment_id")); reason = truncate(message.text or "Rejected by admin", 500)
    payment = await x_payments.find_one({"_id": oid}) if oid else None
    if payment:
        await x_payments.update_one({"_id": oid}, {"$set": {"status": "rejected", "reason": reason, "updated_at": utcnow(), "rejected_by": message.from_user.id}})
        await safe_send_message(int(payment["user_id"]), f"❌ Your X payment was rejected.\nReason: {escape(reason)}")
    await state.clear(); await message.answer("✅ Payment rejected.", reply_markup=main_menu(message.from_user.id))

@router.callback_query(F.data.startswith("xorder:done:"))
async def x_order_done(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback): return
    oid = object_id(callback.data.split(":", 2)[2]); order = await x_orders.find_one({"_id": oid}) if oid else None
    if not order or order.get("status") != "pending":
        await callback.answer("Order not pending.", show_alert=True); return
    ...
    await callback.message.answer("✅ X order marked done.")
    await callback.answer("Done")
...
async def x_order_reject_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(XRejectState.reason); await state.update_data(order_id=oid)
...
async def x_order_reject_reason(message: Message, state: FSMContext) -> None:
    if not await admin_only_message(message): return
    data = await state.get_data(); oid = object_id(data.get("order_id")); reason = truncate(message.text or "Rejected by admin", 500)
    order = await x_orders.find_one({"_id": oid}) if oid else None
    if not order or order.get("status") != "pending":
        await state.clear(); await message.answer("Order not pending.", reply_markup=main_menu(message.from_user.id)); return
    refund_text = ""
    if order.get("pay_type") == "points":
        pts = int(order.get("points", 0) or 0); await users.update_one({"user_id": order["user_id"]}, {"$inc": {"points": pts}}); refund_text = f"Refunded: {pts} points"
    else:
        amount = float(order.get("amount_usdt", 0) or 0); await users.update_one({"user_id": order["user_id"]}, {"$inc": {"balance": amount}}); refund_text = f"Refunded to bot balance: {money(amount)}"
    await x_orders.update_one({"_id": oid}, {"$set": {"status": "rejected", "reason": reason, "updated_at": utcnow(), "rejected_by": message.from_user.id}})
    await safe_send_message(int(order["user_id"]), f"❌ Your X order was rejected.\nReason: {escape(reason)}\n{refund_text}")
    await state.clear(); await message.answer("✅ Order rejected and refunded.", reply_markup=main_menu(message.from_user.id))

@router.callback_query(F.data == "x:myorders")
async def x_my_orders(callback: CallbackQuery) -> None:
    if not await security_check_event(callback): return
    rows=[]; text="🧾 <b>Your X Orders</b>\n\n"
    found=False
    async for order in x_orders.find({"user_id": callback.from_user.id}).sort("created_at", DESCENDING).limit(10):
        found=True; text += f"• {bold(order.get('plan_name'))} | @{escape(order.get('x_username'))} | {bold(order.get('status'))} | {code(short_id(order['_id']))}\n"
    if not found: text += "No X orders yet."
...
async def x_method_view(callback: CallbackQuery) -> None:
    ...
    if await send_force_join_message(callback, "all"):
        return
    user = await get_user(callback.from_user.id) or {}
    title = await get_setting("x_method_title", "X Premium Plus Method")
    price = float(await get_setting("x_method_price_usdt", 10) or 0)
    pts = int(await get_setting("x_method_points", 100) or 0)
    rows: List[List[InlineKeyboardButton]] = []
    if button_visible("x_pay_points", callback.from_user.id):
        rows.append([InlineKeyboardButton(text="💎 Buy Method with Points", callback_data="xmethod:points")])
    if button_visible("x_pay_balance", callback.from_user.id):
        rows.append([InlineKeyboardButton(text="💰 Buy Method with Balance", callback_data="xmethod:balance")])
    rows.append([InlineKeyboardButton(text="⬅️ Back", callback_data="x:back")])
    await callback.message.answer(
        f"📝 {bold(title)}\n\n"
        f"Price: {bold(money(price))} or {bold(pts)} points\n"
        f"Your balance: {bold(money(user.get('balance', 0)))}\n"
        f"Your points: {bold(user.get('points', 0))}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
    )
...
async def x_method_buy(callback: CallbackQuery) -> None:
    ...
    if mode == "points" and not await ensure_button_allowed(callback, "x_pay_points"):
        return
    if mode == "balance" and not await ensure_button_allowed(callback, "x_pay_balance"):
        return
    price = float(await get_setting("x_method_price_usdt", 10) or 0)
    pts = int(await get_setting("x_method_points", 100) or 0)
    if mode == "points":
        res = await users.update_one({"user_id": callback.from_user.id, "points": {"$gte": pts}}, {"$inc": {"points": -pts}})
        if res.modified_count <= 0:
            await callback.answer("Not enough points.", show_alert=True)
            return
    else:
        res = await users.update_one({"user_id": callback.from_user.id, "balance": {"$gte": price}}, {"$inc": {"balance": -price}})
        if res.modified_count <= 0:
            await callback.answer("Not enough balance.", show_alert=True)
            return
    content = await get_setting("x_method_content", "Admin has not added the method yet.")
    await callback.message.answer(f"✅ Method unlocked.\n\n{escape(content)}")
...
async def x_admin_orders(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback):
        return
    found = False
    async for order in x_orders.find({"status": "pending"}).sort("created_at", DESCENDING).limit(15):
        found = True
        text = (
            f"🐦 {bold('Pending X Order')}\n\n"
            f"Order ID: {code(str(order['_id']))}\n"
            f"User: {code(order.get('user_id'))} @{escape(order.get('username') or 'None')}\n"
            f"Plan: {bold(order.get('plan_name'))}\n"
            f"X Username: @{escape(order.get('x_username'))}\n"
            f"Paid with: {bold(order.get('pay_type'))}\n"
            f"Amount: {bold(money(order.get('amount_usdt', 0)))}\n"
            f"Points: {bold(order.get('points', 0))}"
        )
        await callback.message.answer(text, reply_markup=x_order_admin_keyboard(str(order['_id'])))
    if not found:
        await callback.message.answer("✅ No pending X orders.")
    await callback.answer()
...
async def x_admin_payments(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback): return
    found=False
    async for pay in x_payments.find({"status":"pending"}).sort("created_at", DESCENDING).limit(10):
        found=True
        await callback.message.answer(f"💰 X Payment\nID: {code(str(pay['_id']))}\nUser: {code(pay.get('user_id'))}\nPlan: {bold(pay.get('plan_name'))}\nAmount: {bold(money(pay.get('amount_usdt',0)))}\nX: @{escape(pay.get('x_username'))}\nTXID: {code(pay.get('txid'))}", reply_markup=x_payment_admin_keyboard(str(pay['_id'])))
    if not found: await callback.message.answer("No pending X payments.")
    await callback.answer()

@router.callback_query(F.data == "xadmin:settings")
async def x_admin_settings(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback): return
    plans = await get_x_plans(); binance_id = await get_setting("x_binance_id", "")
    ...
    for k,p in plans.items(): text += f"{k}: {p['name']} — {money(p.get('price_usdt',0))} / {p.get('points',0)} pts\n"
...
async def x_setting_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(XSettingsState.value); await state.update_data(key=key)
    await callback.message.answer("Send new value.", reply_markup=cancel_inline_keyboard()); await callback.answer()
...
async def x_setting_save(message: Message, state: FSMContext) -> None:
    ...
    if key == "x_binance_id":
        ...
        plans=await get_x_plans(); parts=key.split("_"); plan_key = {"3m":"3m","6m":"6m","1y":"1y"}.get(parts[1]); field = "price_usdt" if parts[2]=="price" else "points"
        if field == "price_usdt":
            num=parse_float(val)
        else:
            num=parse_int(val)
        if num is None or num < 0:
            await message.answer("Invalid number.", reply_markup=cancel_inline_keyboard()); return
        plans[plan_key][field]=num
        await set_setting("x_plans", plans)
    await state.clear(); await message.answer("✅ X setting saved.", reply_markup=main_menu(message.from_user.id))
...
async def x_method_edit_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(XMethodEditState.value); await state.update_data(field=field)
    await callback.message.answer("Send new value/content.", reply_markup=cancel_inline_keyboard()); await callback.answer()

@router.message(XMethodEditState.value)
async def x_method_edit_save(message: Message, state: FSMContext) -> None:
    if not await admin_only_message(message): return
    data=await state.get_data(); field=data.get("field"); val=message.text or ""
    if field == "x_method_price_usdt":
        num=parse_float(val)
        if num is None: await message.answer("Invalid price."); return
        await set_setting(field, num)
    elif field == "x_method_points":
        num=parse_int(val)
        if num is None: await message.answer("Invalid points."); return
        await set_setting(field, num)
    else:
        await set_setting(field, val)
    await state.clear(); await message.answer("✅ X method updated.", reply_markup=main_menu(message.from_user.id))
