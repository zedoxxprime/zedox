...
async def x_premium_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"Balance: {bold(money(user.get('balance', 0)))}\n"
        f"Points: {bold(user.get('points', 0))}\n\n"
        f"Choose a plan. You can buy using points or USDT/manual payment.",
        reply_markup=await x_plan_keyboard(plans, message.from_user.id),
    )
...
async def x_back(callback: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await x_premium_menu(callback.message, state)
    await callback.answer()

@router.callback_query(F.data.startswith("xbuy:points:") | F.data.startswith("xbuy:balance:"))
...
async def x_method_view(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(f"📝 {bold(title)}\n\nPrice: {bold(money(price))} or {bold(pts)} points\nYour balance: {bold(money(user.get('balance', 0)))}\nYour points: {bold(user.get('points', 0))}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def x_method_buy(callback: CallbackQuery) -> None:
    if not await security_check_event(callback): return
    mode = callback.data.split(":")[1]; user = await get_user(callback.from_user.id) or {}
    price = float(await get_setting("x_method_price_usdt", 10) or 0); pts = int(await get_setting("x_method_points", 100) or 0)
    if mode == "points":
        res = await users.update_one({"user_id": callback.from_user.id, "points": {"$gte": pts}}, {"$inc": {"points": -pts}})
        if res.modified_count <= 0: await callback.answer("Not enough points.", show_alert=True); return
    else:
        res = await users.update_one({"user_id": callback.from_user.id, "balance": {"$gte": price}}, {"$inc": {"balance": -price}})
        if res.modified_count <= 0: await callback.answer("Not enough balance.", show_alert=True); return
