...
def main_menu(user_id: int) -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(text="🛍️ PRODUCTS"), KeyboardButton(text="🎁 FREE CANVA")],
        [KeyboardButton(text="📝 PREORDER"), KeyboardButton(text="🎁 FREE ITEMS")],
        [KeyboardButton(text="💳 METHODS"), KeyboardButton(text="👥 REFERRAL")],
        [KeyboardButton(text="💰 DEPOSIT")],
        [KeyboardButton(text="📞 SUPPORT")],
        [KeyboardButton(text="💼 BALANCE"), KeyboardButton(text="💱 EXCHANGE POINTS")],
...
async def product_buy_keyboard(pid: str, stock_count: int) -> InlineKeyboardMarkup:
    rows = []
    rows.append([InlineKeyboardButton(text="Buy 1", callback_data=f"buy:{pid}:1")])
    for rule in await get_bulk_discount_rules():
        quantity = int(rule.get("quantity", 0))
        percent = int(rule.get("percent", 0))
        if quantity > 1 and stock_count >= quantity:
            rows.append([InlineKeyboardButton(text=f"Buy {quantity} — {percent}% OFF", callback_data=f"buy:{pid}:{quantity}")])
...
USER_MENU_TEXTS = {
    "🛍️ PRODUCTS", "🎁 FREE CANVA", "👥 REFERRAL", "💳 METHODS",
    "💰 DEPOSIT", "📞 SUPPORT", "💼 BALANCE", "🧾 MY ORDERS",
    "🎁 FREE ITEMS", "📝 PREORDER", "👑 ADMIN PANEL",
}
...
async def send_preorder_payment_methods(target: Message, state: FSMContext) -> bool:
    ...
    await target.answer(
        ...
        "Choose a payment method, pay the preorder amount, then upload your payment screenshot.",
...
async def create_preorder_after_payment(message: Message, state: FSMContext, screenshot_file_id: str, screenshot_type: str) -> None:
    ...
    caption = (
        ...
        f"Advance paid/total: {bold(money(total))}\n"
        f"Payment method: {bold(method_name)}\n\n"
        f"Details:\n{escape(details)}"
    )
    for admin_id in list(dict.fromkeys(ADMIN_IDS)):
    ...
    await audit("paid_preorder_created", message.from_user.id, {"preorder_id": preorder_id, "type": preorder_type, "total": total})
...
async def preorder_start(message: Message, state: FSMContext) -> None:
    ...
    found = False
    await message.answer("📝 <b>Preorder Accounts</b>\n\nChoose a preorder product below, or use manual preorder.")
    async for pp in preorder_products.find({"active": {"$ne": False}}).sort([("position", ASCENDING), ("created_at", DESCENDING)]):
        ...
        await message.answer(
            ...
            f"💵 Price per account: {bold(money(pp.get('price', 0)))}\n"
            f"📌 Status: {bold('Available for preorder')}\n\n"
            f"{escape(pp.get('description', ''))}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        )

    manual_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✍️ Manual Preorder", callback_data="preorder:manual")],
        [InlineKeyboardButton(text="❌ Cancel", callback_data="global:cancel")],
    ])
    if not found:
        await message.answer(
            "No preorder products added yet. You can still send a manual preorder.",
            reply_markup=manual_keyboard,
        )
    else:
        await message.answer("Need another product?", reply_markup=manual_keyboard)
...
async def preorder_save(message: Message, state: FSMContext) -> None:
    ...
    await send_preorder_payment_methods(message, state)

@router.callback_query(F.data.startswith("prepaymethod:"))
async def preorder_payment_method_selected(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not method:
        ...
        return
    await state.update_data(payment_method_id=mid, payment_method_name=method.get("name"))
...
async def admin_preorders(callback: CallbackQuery) -> None:
    if not await admin_only_callback(callback):
        return
    found = False
    async for pre in preorders.find({"status": {"$in": ["pending", "answered"]}}).sort("created_at", DESCENDING).limit(20):
        found = True
        pid = str(pre["_id"])
        await callback.message.answer(
            f"📝 {bold('Preorder')}\n\n"
...
async def preorder_done(callback: CallbackQuery) -> None:
    ...
    await safe_send_message(pre["user_id"], f"✅ Your preorder {code(short_id(pre['_id']))} has been fulfilled. Check admin message or contact support if needed.")
...
async def preorder_reject(callback: CallbackQuery) -> None:
    ...
    if not pre:
        await callback.answer("Preorder not found.", show_alert=True)
        return
    await preorders.update_one({"_id": pre["_id"]}, {"$set": {"status": "rejected", "handled_by": callback.from_user.id, "updated_at": utcnow()}})
    await safe_send_message(pre["user_id"], f"❌ Your preorder {code(short_id(pre['_id']))} was rejected. Contact support for details.")
    await callback.message.answer("❌ Preorder rejected and client notified.")
    await callback.answer()
...
async def show_products(message: Message) -> None:
    ...
    # In-stock products are shown first by manual position, then name.
    # Stockout products are still visible, but always move to the last section.
    product_list.sort(key=lambda p: (0 if p.get("_stock_count", 0) > 0 else 1, int(p.get("position", 999999)), str(p.get("name", "")).lower()))

    async def product_callback_for_button(product: Dict[str, Any]) -> str:
        return f"product:view:{product['_id']}"

    for product in product_list:
        stock_count = int(product.get("_stock_count", 0))
        if stock_count <= 0:
            label = f"{product.get('name')} — Stockout"
        else:
            label = f"{product.get('name')} ({stock_count})"
        current.append(InlineKeyboardButton(text=label, callback_data=f"product:view:{product['_id']}"))
        if len(current) == 2:
            rows.append(current)
            current = []
    if current:
...
async def user_view_product(callback: CallbackQuery) -> None:
    ...
    if stock_count <= 0:
        await callback.message.answer(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📛 Stockout", callback_data="noop:stockout")], [InlineKeyboardButton(text="❌ Cancel", callback_data="global:cancel")]]),
        )
    else:
        await callback.message.answer(
            await format_product_display(product),
            reply_markup=await product_buy_keyboard(pid, stock_count),
        )
    await callback.answer()
...
async def buy_product(callback: CallbackQuery) -> None:
    ...
    if not quantity or quantity <= 0 or quantity > 100:
...
async def admin_users(callback: CallbackQuery) -> None:
    ...
    cursor = users.find({}).sort("created_at", DESCENDING).skip(skip).limit(limit)
...
async def setting_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    prompts = {
        "support_username": "Send default product contact username without @:",
        ...
        "force_join_channels": "Send required channels separated by comma (example: @channel1,@channel2). Send empty/off to disable channels:",
        "force_join_scope": "Send scope: none, all, or free_canva",
        "bulk_discount_rules": "Send bulk discounts like: 2=5,5=10,10=15\nThis means: buy 2 get 5% off, buy 5 get 10% off, buy 10 get 15% off.",
        "points_per_usdt": "Send exchange rate like: 100:1\nThis means: 100 points = 1 USDT.\nYou can also send 500:5, 1000:10, etc.",
    }
    await callback.message.answer(prompts.get(key, "Send value:"))
    await callback.answer()
...
async def show_free_products(message: Message) -> None:
    ...
    if not rows:
        await message.answer("No free items available yet.")
        return
    await message.answer(f"🎁 {bold('Free Items')}\n\nUse referral points or USDT balance.", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def add_free_product_desc(message: Message, state: FSMContext) -> None:
    if not await admin_only_message(message):
        return
    data = await state.get_data()
    result = await free_products.insert_one({"name": data["name"], "price": data["price"], "points": data["points"], "description": truncate(message.text or '', MAX_PRODUCT_DESCRIPTION), "stock": [], "active": True, "sales": 0, "revenue": 0.0, "created_at": utcnow(), "updated_at": utcnow()})
    await state.clear()
...
async def finish_content_method_delivery(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not items:
        ...
        return
    result = await content_methods.insert_one({"name": data["name"], "price": float(data["price"]), "description": data["description"], "delivery_type": "multi", "delivery": "", "delivery_items": items, "position": await next_position(content_methods), "active": True, "sales": 0, "revenue": 0.0, "created_at": utcnow(), "updated_at": utcnow()})
    await state.clear()
...
async def list_content_methods_admin(callback: CallbackQuery) -> None:
    ...
    async for m in content_methods.find({}).sort([("position", ASCENDING), ("created_at", DESCENDING)]):
        found = True
        await callback.message.answer(f"🎬 {bold(m.get('name'))}\nID: {code(m['_id'])}\nPrice: {bold(money(m.get('price', 0)))}\nType: {bold(m.get('delivery_type', 'text'))}\nActive: {bold(m.get('active', True))}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
@router.message(F.text == "💱 EXCHANGE POINTS")
...
async def preorder_manual_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        "Send what account/product you need, quantity, duration, and any note.\n\n"
        "Example:\n<code>ChatGPT Plus, 2 accounts, 30 days</code>",
        reply_markup=cancel_inline_keyboard(),
    )
    await callback.answer()
...
async def preorder_product_selected(callback: CallbackQuery) -> None:
    ...
    if not qty or qty <= 0:
        await callback.answer("Invalid quantity.", show_alert=True)
        return
    total = round(float(pp.get("price", 0)) * qty, 2)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Send Preorder to Admin", callback_data=f"preprod_confirm:{pid}:{qty}")],
        [InlineKeyboardButton(text="❌ Cancel", callback_data="global:cancel")],
    ])
    await callback.message.answer(
        ...
        f"Accounts: {bold(qty)}\n"
        f"Price each: {bold(money(pp.get('price', 0)))}\n"
        f"Estimated total: {bold(money(total))}\n\n"
        f"Click approve to send this preorder to admin.",
        reply_markup=keyboard,
    )
    await callback.answer()
...
async def preorder_product_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not pp or pp.get("active") is False or not qty:
        ...
        return
    total = round(float(pp.get("price", 0)) * qty, 2)
    details = f"Catalog preorder: {pp.get('name')} | Quantity: {qty} account(s) | Advance total: {money(total)}"
    await state.update_data(
        preorder_type="catalog",
        details=details,
        product_id=str(pp["_id"]),
        product_name=pp.get("name"),
        quantity=qty,
        price_each=float(pp.get("price", 0)),
        estimated_total=total,
    )
    await callback.message.answer(
        f"💰 <b>Advance Payment Required</b>\n\n"
        f"Product: {bold(pp.get('name'))}\n"
        f"Accounts: {bold(qty)}\n"
        f"Total to pay now: {bold(money(total))}\n\n"
        f"Choose payment method and upload screenshot to submit preorder."
    )
